...
const smokeExpression = String.raw`(async () => {
  ...
  try {
    ...
    const t3Window = snapshot?.accessibility?.windows.find(
      (candidate) => candidate.application === "t3code",
    );
    if (t3Window === undefined) throw new Error("the initial window list omitted T3 Code");
    const focused = valueOf(
      await computer.act({
        actions: [
          { type: "activate_window", windowId: t3Window.id },
    ...
    if (!document.hasFocus()) throw new Error("T3 Code did not gain document focus");
    if (!wasMaximized) {

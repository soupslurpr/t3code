...
async function closePortalSession(closingHandle, reportFailure) {
  try {
    ...
    const alreadyClosed =
      message.includes("org.freedesktop.DBus.Error.UnknownObject") ||
      message.includes("org.freedesktop.DBus.Error.UnknownMethod: Object does not exist");
...
/** Maps one compositor position to its selected monitor stream. */
function resolveStreamPosition(x, y) {
  ...
  if (selected === null) {
    const error = new Error(`desktop position (${x}, ${y}) is not on a shared monitor`);
    error.code = "screen-not-shared";
    throw error;
  }
  const [left, top] = selected.position ?? [0, 0];
  const localX = x - left;
  const localY = y - top;
  if (
    selected.size !== null &&
    (localX < 0 || localY < 0 || localX >= selected.size[0] || localY >= selected.size[1])
  ) {
    const error = new Error(`desktop position (${x}, ${y}) is not on the shared monitor`);
    error.code = "screen-not-shared";
    throw error;
  }
  return { stream: selected, x: localX, y: localY };
}
...
async function movePointer(x, y, durationMs, requestedSteps) {
  requireDevice(POINTER_DEVICE, "pointer");
  const target = resolveStreamPosition(x, y);
  ...
  switch (message.method) {
    ...
    case "move": {
      ...
      await runInputPhase("pointer-move", () =>
        movePointer(message.params.x, message.params.y, message.params.durationMs),
      );
      return null;
    }
    case "click": {

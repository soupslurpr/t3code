...
describe("deriveTimelineEntries", () => {
  ...
  it("keeps monitor events visible when question answers fold into the work log", () => {
    ...
    const messages = [
      ...
      {
        ...streamingMessage,
        id: MessageId.make("async-answer:question-request"),
        role: "user" as const,
        text: "Proceed",
        streaming: false,
      },
      {
        ...streamingMessage,
        id: MessageId.make("internal-continuation"),
        role: "system" as const,
        text: "Internal continuation",
        streaming: false,
      },
    ];
    const beforeAnswerActivity = deriveTimelineEntriesWithState(messages, [], [], streamed);
    expect(beforeAnswerActivity.entries.map((entry) => entry.id)).toEqual([
    ...
    const folded = deriveTimelineEntriesWithState(messages, [], work, beforeAnswerActivity);
    expect(folded.entries.map((entry) => entry.id)).toEqual([
      "monitor-event-message",
      "streaming-message",
      "answer-submitted",
    ]);
    expect(folded.entries[0]).toEqual(monitorEntry);
  });

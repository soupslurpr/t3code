...
function DesktopViewer({
  ...
  const releaseHeldKeys = useCallback(() => {
    if (!controlling || heldKeysRef.current.size === 0) return;
  ...
  const takeControl = useCallback(async () => {
    ...
    try {
      ...
      if (!granted || document.fullscreenElement === null) {
        if (granted) await onRelease();
  ...
  }, [controlling, onRelease, releaseHeldKeys, releaseKeyboardCapture]);

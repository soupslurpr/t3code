...
describe("ProviderSessionReaper", () => {
    ...
    const layer = makeProviderSessionReaperLive({
      ...
      Layer.provideMerge(
        Layer.succeed(ProjectionSnapshotQuery, {
          ...
          getThreadTurnStartContext: () => Effect.die("unused"),
  ...
  it("stops an orphaned projected active turn during startup reconciliation", async () => {
    ...
    await startReaper();
    await waitFor(() =>
      harness.dispatchedCommands.some((command) => command.type === "thread.session.set"),
    );

    expect(harness.stopSession).toHaveBeenCalledWith({ threadId });
    const command = harness.dispatchedCommands.find((entry) => entry.type === "thread.session.set");

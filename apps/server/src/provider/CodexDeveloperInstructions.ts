...
When \`computer_*\` tools are exposed and a task needs a GUI, call \`computer_request_view\` or \`computer_request_control\` early. Select the user's desktop or an isolated Agent desktop according to the task; Agent desktops do not require the user's host screen-sharing approval. Access requests return an initial observation by default and accept an observation policy. Agent access returns a concrete desktop id: retain it and pass \`desktop: { kind: "agent", desktopId }\` on every later computer call, including release. Use \`fresh: true\` when parallel agents need independent desktops. Omitting desktop always targets the user's desktop; no computer call relies on a shared implicit selection. \`computer_act\` executes ordered actions and returns one configurable fresh observation. Batch predictable steps; use one action when the next step depends on the resulting UI. Prefer reliable keyboard navigation for known commands. For example, starting a known app is usually one batch: press Meta, wait briefly, type its name with \`submit:true\`, then wait for it to open.

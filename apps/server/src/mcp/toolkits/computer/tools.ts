...
export const ComputerActTool = computerTool(
  Tool.make("computer_act", {
    description:
      "Run one through 32 ordered native desktop actions, then return ordered actionResults and a configurable fresh observation. Omission targets the user's desktop; always pass a concrete Agent desktopId returned by access. Actions support click, move, activate, activate_window, drag, wheel, type, press, hotkey, key_down, key_up, wait, and wait_for_change. Fixed wait is limited to 5000ms; wait_for_change is limited to 60000ms and reports changes after the wait starts, using a fresh baseline; frameId defines coordinates, not the baseline. Changes that finished before the wait are not detected. Batch predictable actions that do not need an intermediate visual decision; use a one-action batch when the next step depends on the resulting UI. Type preserves exact Unicode without changing the clipboard, fails explicitly when no exact delivery path exists, and reports injected versus accessibility-confirmed code points so a stale rendering is not mistaken for lost text. Pointer coordinates reference a returned frame id, preserving its crop, resolution, and transform. Wheel emits discrete hardware-like ticks. Hotkey presses a chord atomically and releases acquired keys; key_down/key_up remain available for deliberate holds such as inspecting Alt+Tab. A semantic target or window activation must be first and only one semantic activation is allowed; capture a fresh observation immediately before it because semantic ids are ephemeral and an Agent-desktop action batch consumes them. Set observation false after predictable actions, or choose screenshot resolution, crop, accessibility, and delay. Failures visibly report the exact field, action index, completed count, phase, expected value, and input cleanup result.",
